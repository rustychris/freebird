...
{
protected:
public: // while working out the async iface
 ...
 public:
  ...
  uint16_t conversion_config(uint8_t channel,uint8_t single_ended);
  void triggerADC_SingleEnded(uint8_t channel);
  void triggerADC_SingleEnded_async(uint8_t channel);
  uint16_t  readADC_SingleEnded(uint8_t channel);
  uint16_t  elidedADC_SingleEnded(uint8_t channel);

  int16_t   readADC_Differential_0_1(void);
  int16_t   readADC_Differential_2_3(void);
  void      startComparator_SingleEnded(uint8_t channel, int16_t threshold);
  int16_t   getLastConversionResults();
  uint16_t   getLastConversionResultsSingleEnded();
